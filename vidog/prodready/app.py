...
GALLERY_V2_DIR = os.path.join(CUR_DIR, "data", "gallery_v2")
NOSE_DETECTOR_PATH = os.path.join(CUR_DIR, "checkpoints", "nose_detector.onnx")
...
with tab_register:
    ...
    with col1:
        ...
        gender = st.selectbox("Gender", options=["Female", "Male", "Unknown"])
...
with tab_identify:
    ...
    id_gender = st.selectbox("Filter by Gender", options=["All", "Female", "Male", "Unknown"], key="id_gender")
        ...
        with col_match:
            if st.button("?? Run Hybrid Identification", type="primary"):
                with st.spinner("Embedding query and querying hybrid classifier..."):
                    ...
                    match_gender = None if id_gender == "All" else id_gender
                    top_match, all_candidates = matcher.match(emb_nose, emb_face, gender=match_gender)
                ...
                if all_candidates:
                    ...
                    for c in all_candidates:
                        table_data.append({
                            "Dog": c.dog_name,
                            "Gender": c.gender if c.gender else "N/A",
                            "Status": c.status,
...
with tab_gallery:
    ...
    if not os.path.exists(GALLERY_V2_DIR):
        ...
        for dog in dogs:
            ...
            with st.expander(f"?? {meta.get('dog_name', dog)}  ({meta.get('breed', 'Unknown')}, {meta.get('gender', 'N/A')}, {meta.get('frame_count', '?')} frames)"):
                st.json(meta)
